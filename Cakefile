...
launch = (cmd, options=[], callback) ->
    ...
    exec "coffee #{options.join(" ")}", (error, stdout, stderr) ->
...
build = (watch, callback) ->
    unless building
        ...
        options = ["-j js/#{application}.js", '-c', '-b', '-o' ]
        options = options.concat files
        continues_building() if watch
...
watch = (file) ->
    fs.watch file, (curr, prev) ->
        if +curr.mtime isnt +prev.mtime
            build false, -> log "#{file} rebuild [ok]", green
